...
function postComment(commentDets) {          
    ...
    function displayComment(data) {
                ...
                var html = '<div class="panel panel-white post panel-shadow"><div class="post-heading"><button type="button" class="close" onclick="deleteComment(\''+data._id+'\');" aria-label="Close"><span aria-hidden="true">&times;</span></button><div class="pull-left image"><img src="http://bootdey.com/img/Content/user_1.jpg" class="img-circle avatar" alt="user profile image"></div><div class="pull-left meta"><div class="title h5"><a href="#"><b>'+data.handle+'</b></a></div><h6 class="text-muted time">'+stringTimeElapsed+'</h6></div></div><div class="post-description"><p>'+data.comment+'</p><div class="stats"><button onclick="upVotes(\''+data._id+'\');" class="btn btn-default stat-item" id="upVotes"><i class="fa fa-thumbs-up icon"></i>'+data.upVotes+'</button><button onclick="downVotes(\''+data._id+'\');" class="btn btn-default stat-item" id="downVotes" ><i class="fa fa-thumbs-down icon"></i>'+data.downVotes+'</button></div></div></div>';
